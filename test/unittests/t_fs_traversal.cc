...
class T_FsTraversal : public ::testing::Test {
 public:
  struct Checklist {
    ...
    Checklist() : type(Unspecified) { Init(); }
    ...
    void Check(const Type overwrite_type = Unspecified) const {
      const Type type_to_check = (overwrite_type != Unspecified)
                                     ? overwrite_type
                                     : type;
...
class BaseTraversalDelegate {
 ...
 protected:
  ...
  void fail(const std::string msg) const { FAIL() << msg; }
...
TEST_F(T_FsTraversal, FullTraversal) {
  ...
  FileSystemTraversal<BaseTraversalDelegate> traverse(
      &delegate, testbed_path_, true);
...
TEST_F(T_FsTraversal, RootTraversal) {
  ...
  FileSystemTraversal<RootTraversalDelegate> traverse(
      &delegate, testbed_path_, false);
  RegisterDelegate(&traverse);
...
TEST_F(T_FsTraversal, IgnoringTraversal) {
  ...
  FileSystemTraversal<IgnoringTraversalDelegate> traverse(
      &delegate, testbed_path_, true);
  RegisterDelegate(&traverse);
